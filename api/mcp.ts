...
/**
 * Extract Meta access token from request. Client passes token per connection via:
 * - X-Meta-Access-Token header, or
 * - Authorization: Bearer <token>
 * Fallback: META_ACCESS_TOKEN or META_MCP_TOKEN env.
 */
function getAuthFromRequest(req: Request): AuthManager | null {
  const token =
    req.headers.get("x-meta-access-token") ||
    (req.headers.get("authorization")?.startsWith("Bearer ")
      ? req.headers.get("authorization")!.slice(7)
      : null) ||
    process.env.META_ACCESS_TOKEN ||
    process.env.META_MCP_TOKEN;
